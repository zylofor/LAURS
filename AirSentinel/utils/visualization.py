...
from config.rudet import class_table
from config.rudet import radar_configs, rudet_configs
...
def visualize_train_img(fig_name, input_radar, output_confmap_rarvav, output_confmap_rvav, confmap_gt):
    ...
    output_confmap = np.transpose(output_confmap_rarvav, (1, 2, 0))
    output_confmap[output_confmap < 0] = 0
    plt.imshow(output_confmap, vmin=0, vmax=1, origin='lower', aspect='auto')

    fig.add_subplot(2, 2, 3)
    output_confmap = np.transpose(output_confmap_rvav, (1, 2, 0))
    output_confmap[output_confmap < 0] = 0
    plt.imshow(output_confmap, vmin=0, vmax=1, origin='lower', aspect='auto')
...
def visualize_test_img(fig_name, img_path, input_radar, confmap_pred, confmap_gt, res_final, sybl=False):
    ...
    colors = ['white', 'yellow']  # 白色 和 浅黄色
    conf_list = [0.38, 0.57]
    plt.imshow(confmap_pred.squeeze(2), vmin=0, vmax=1, origin='lower', aspect='auto')
    for d in range(rudet_configs['max_dets']):
        cla_id = int(res_final[d, 0])
        if cla_id == -1:
            continue
        row_id = res_final[d, 1]
        col_id = res_final[d, 2]
        conf = res_final[d, 3] + conf_list[d]
        conf = 1.0 if conf > 1 else conf
        cla_str = class_table[cla_id]
        if sybl:
            text = symbols[cla_str]
            plt.text(col_id, row_id+3, text, fontproperties=fp, color=colors[d], size=20, ha="center")
        else:
            plt.scatter(col_id, row_id, s=10, c=colors[d])
            text = cla_str.upper() + '\n%.2f'%conf
            plt.text(col_id+5, row_id, text, color=colors[d], fontsize=10)

    plt.axis('off')
    plt.title("radarUAVDet Detection")

    fig.add_subplot(2, 2, 4)
    confmap_gt = np.transpose(confmap_gt, (1, 2, 0))
    plt.imshow(confmap_gt.squeeze(2), vmin=0, vmax=1, origin='lower', aspect='auto')
    plt.axis('off')
