...
from utils.mappings import labelmap2ra, confmap2ra
from utils.visualization import visualize_confmap
from config.rudet import n_class, class_table, radar_configs, confmap_sigmas, rudet_configs, confmap_length, confmap_sigmas_interval
...
def generate_confmap(obj_info, gaussian_thres=36, type='ra'):
    ...
    """
    if type == 'ra':
        confmap = np.zeros((n_class, int(radar_configs['rvaemap_rsize'] // rudet_configs['r_rate']), int(radar_configs['rvaemap_asize'] // rudet_configs['a_rate'])), dtype=float)
        for rng_idx, agl_idx, class_id in obj_info:
            ...
            for i in range(int(radar_configs['rvaemap_rsize'] // rudet_configs['r_rate'])):
                for j in range(int(radar_configs['rvaemap_asize'] // rudet_configs['a_rate'])):
                    # 计算当前索引点 (i, j) 与中心点 (rng_idx, agl_idx) 的距离，并对距离进行归一化处理
                    distant = (((rng_idx - i) * 2) ** 2 + (agl_idx - j) ** 2) / sigma**2

                    # 如果距离小于阈值，意味着当前点可能属于目标区域，因此需要计算置信度值
                    if distant < gaussian_thres:  # threshold for confidence maps
                        value = np.exp(- distant / 2) / (2 * math.pi)

                        # 制作confmap:如果计算得到的value比当前confmap中的值大，就更新confmap中该位置的值；否则保持原值不变。
                        confmap[class_id, i, j] = value if value > confmap[class_id, i, j] else confmap[class_id, i, j]
    else:
        confmap = np.zeros((n_class, int(radar_configs['rvaemap_rsize'] // rudet_configs['r_rate']), int(radar_configs['rvaemap_vsize'] // rudet_configs['v_rate'])), dtype=float)
        for rng_idx, agl_idx, dop_idx, class_id in obj_info:
            ...
            if type == 'rv':
                for i in range(int(radar_configs['rvaemap_rsize'] // rudet_configs['r_rate'])):
                    for j in range(int(radar_configs['rvaemap_vsize'] // rudet_configs['v_rate'])):
                        distant = (((rng_idx - i) * 2) ** 2 + ((dop_idx - j) * 2) ** 2) / sigma ** 2
                        if distant < gaussian_thres:  # threshold for confidence maps
                            value = np.exp(- distant / 2) / (2 * math.pi)
                            confmap[class_id, i, j] = value if value > confmap[class_id, i, j] else confmap[class_id, i, j]
            elif type == 'av':
                for i in range(int(radar_configs['rvaemap_asize'] // rudet_configs['a_rate'])):
                    for j in range(int(radar_configs['rvaemap_vsize'] // rudet_configs['v_rate'])):
...
def add_noise_channel(confmap):
    confmap_new = np.zeros((n_class + 1, int(radar_configs['rvaemap_rsize'] // rudet_configs['r_rate']), int(radar_configs['rvaemap_asize'] // rudet_configs['a_rate'])), dtype=float)
...
def vehicle_confmap(raloc, width, length, ori, gaussian_thres=10):
    ...
    n_segs = len(segments)
    confmap = np.zeros((int(radar_configs['rvaemap_rsize'] // rudet_configs['r_rate']), int(radar_configs['rvaemap_asize'] // rudet_configs['a_rate'])), dtype=float)
    pps = []

    for segid in range(n_segs):
        for ri in range(int(radar_configs['rvaemap_rsize'] // rudet_configs['r_rate'])):
            for aj in range(int(radar_configs['rvaemap_asize'] // rudet_configs['a_rate'])):
